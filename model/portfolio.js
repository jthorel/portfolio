	...
	this.projects = [
		{
			...
			desc:
				"A competitive top-down pixel-art action local multiplayer game developed in Unity3D for PC and Mac. Did most of the programming together with another team member. Design decisions and ideas toghether with whole team. Created and presented a satirical trailer.",
		...
		},
		{
			...
			desc:
				"Split screen car game on breakable ice, created in Unreal Engine during Nordic Game Jam 2018. Drive around, use power-ups, bump and break the ice for the other player while avoiding going through the ice",
...

		{
			...
			desc:
				'Master thesis project. Created a VR prototype for collaborative planning of roads, with user studies. Made in Unreal Engine. Prototype is fully functional with "multiplayer" support.',
...

		{
			...
			desc:
				"Line Rider in Augmented Reality. Developed in Unity3D using ARKit on iOS. Draw lines in the real world and ride them on your phone.",
		...
		},
		{
			...
			desc:
				"Bachelor thesis. Iphone app using spatial sound GPS beacons for navigation, with user testing. ",
		...
		},
		{
			...
			desc:
				"Webapp to save sets of music. Created for a interaction programming course. Fully functional with user functions and a document-oriented (MongoDB) database.",
		...
		},
		{
			...
			desc:
				"Create activities using geolocation. Users can join by checking nearby activities or byt using the map. User functions and chat function in the activity. Not actively maintained, some function might not work.",
		...
		},
		{
			...
			desc:
				"Visualize where traffic accidents happened in live view. More details are also available when the pin is pressed. Prototype built in a day in Swift and ARKit, with real data from Folksam.",
...

		{
			...
			desc:
				"Redesign of a course planning system with an interactive prototype",
			tags: [
				"Interaction Design",
				"Prototyping",
				"UX",
			...
			link: {
				url:
					"http://xml.csc.kth.se/~jthorel/ixd-metod/prototyp/schedule_fin.html",
				text: "Test prototype",
			},
		},

		{
			...
			thumb: "student1.png",
			shortdesc:
				"Folksam student home insurance app, released to customers",
		...
		},
		{
			...
			desc:
				"Tracks water intake and when you should drink. Talks to an app that show how much you have drank, how much water in bottle, and when you should drink. Made with Arduino with different sensors (accelerometer, gyro, pressure, wifi module)",
			tags: [
				"Arduino",
				"C++",
		...
		for (var i in this.projects) {
			if (this.projects[i].id == id) {
				if (this.projects[i].comments.length < 1) {
					...
					var newid =
						this.projects[i].comments[
							this.projects[i].comments.length - 1
						].id + 1;
					commentObj.id = newid;
					this.projects[i].comments.push(commentObj);
					break;
